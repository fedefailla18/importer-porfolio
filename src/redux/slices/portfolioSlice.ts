  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      try {
        ...
        console.warn(
          'Portfolio names endpoint not available, using default list'
        )
...
export const fetchPortfolioDetails = createAsyncThunk(
  ...
  async (portfolioName: string, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        error.response?.data || 'Failed to fetch portfolio details'
      )
    }
  }
)
  ...
  async (
    {
      ...
      portfolioName,
      symbols,
    }: { file: File; portfolioName?: string; symbols?: string[] },
    { rejectWithValue }
  ) => {
    try {
      ...
      return rejectWithValue(
        error.response?.data || 'Failed to upload transactions'
      )
    }
  }
)
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchPortfolioDetails.rejected, (state, action) => {
        state.status = 'failed'
        state.error =
          action.error.message || 'Failed to fetch portfolio details'
      })
  },
})
